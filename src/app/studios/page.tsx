...
export default function StudiosPage() {
  ...
  const handleOpenKakaoMap = (lab: PartnerLabItem) => {
    const query = encodeURIComponent(${lab.name} );
    window.open(https://map.kakao.com/link/search/, '_blank');
  };
    ...
    <div className=max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10>
            ...
            <div className=pt-2 border-t border-vintage-100 flex items-center gap-2>
              <button
                ...
                onClick={() => {
                  ...
                  addCoupon({
                    id: 'coupon-lab-' + Date.now(),
                    title: [현장 20% 할인] ,
                    issuerName: lab.name,
                    discountText: lab.discountText,
                    validUntil: '2026.12.31',
                    category: 'lab',
                    isUsed: false,
                  });
                  showToast(${lab.name} 20% 할인권이 발급되어 내 쿠폰함에 보관되었습니다!, 'success');
                }}
                className=flex-1 py-3 rounded-xl bg-vintage-900 hover:bg-terracotta text-white text-xs font-bold transition-all shadow-xs flex items-center justify-center gap-1.5 active:scale-95
              >
              ...
              <button
                ...
                className=px-4 py-3 rounded-xl border border-vintage-300 hover:bg-vintage-100 text-vintage-800 text-xs font-semibold transition-colors flex items-center gap-1
                title=카카오맵으로 길찾기
              >
              ...
              <button
                ...
                className=p-3 rounded-xl bg-amber-300 hover:bg-amber-400 text-stone-900 font-bold transition-colors shadow-2xs
                title=카카오톡 공유
              >
      ...
      {selectedStudioForQr && (
        <QrCouponModal
          ...
          categoryName={selectedStudioForQr.badge}
